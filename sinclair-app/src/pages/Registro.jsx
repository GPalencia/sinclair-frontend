...
export default function Registro() {
  ...
  useEffect(() => {
    ...
    setLaborSel(l)
    if (l?.pideDias && l?.valorDiario) setDias(1)
    setValor('')
  }, [laborId])
  ...
  async function guardar() {
    ...
    if (laborSel?.pideDias && laborSel?.valorDiario) {
      salario = dias * laborSel.valorDiario
  ...
  const salarioCalc = laborSel?.pideDias && laborSel?.valorDiario ? dias * laborSel.valorDiario : null
        ...
        <div className="card fade-up" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {laborSel?.pideDias && (
            <div>
              ...
              <input className="inp" type="number" min={0.5} max={2} step={0.5} value={dias} onChange={e => setDias(parseFloat(e.target.value))} />
            </div>
          )}

          {/* Valor manual (destajo) */}
          {laborSel?.pideValor && !laborSel?.pideDias && (
            <div>
              <label className="lbl">Monto a pagar (Lps) *</label>
              <input className="inp" type="number" min={0} step={0.01} placeholder="0.00" value={valorManual} onChange={e => setValor(e.target.value)} />
            </div>
          )}

          {/* Salario calculado */}
          {salarioCalc !== null && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--surface)', border: '1px solid rgba(34,197,94,.2)', borderRadius: 8, padding: '.75rem 1rem' }}>
              <span style={{ fontSize: '.82rem', color: 'var(--muted)' }}>Salario calculado:</span>
              <span style={{ fontFamily: 'DM Mono, monospace', fontSize: '1.1rem', color: 'var(--verde)', fontWeight: 700 }}>
                L {salarioCalc.toFixed(2)}
              </span>
            </div>
          )}
